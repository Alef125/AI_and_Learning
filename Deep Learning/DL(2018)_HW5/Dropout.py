...
with tf.name_scope("conv_layer_1"):
    ...
    conv1_out = existence * dropout(tf.nn.relu(conv1 + b_h1), prob=keep_prob)
...
with tf.name_scope("conv_layer_2"):
    ...
    conv2_out = existence * dropout(tf.nn.relu(conv2 + b_h2), prob=keep_prob)
...
with tf.Session() as sess:
    ...
    for i in range(2000):
        ...
        if i % 200 == 0:
            ...
            current_loss, accuracy = sess.run((loss, accuracy_train),
                                              feed_dict={x: x_sample, y: y_sample,
    ...
    # x_sample = tf.reshape(x_sample, shape=(1, 28, 28, 1))
    x_sample = np.reshape(x_sample, newshape=(check_batch, 28, 28, 1))
    accuracy_test = sess.run(accuracy_train,
                             feed_dict={x: x_sample, y: y_sample,
                                        keep_prob_input: 1.0, keep_prob: 1.0,
                                        in_existence: input_prob, existence: neuron_prob}
                             )
