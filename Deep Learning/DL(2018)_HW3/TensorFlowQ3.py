...
with tf.name_scope(name="Hidden_Layer"):
    w1 = tf.Variable(tf.random_uniform(shape=(784, 500), minval=-1, maxval=1), name="w1")
    b1 = tf.Variable(tf.zeros(500), name="b1")
    z1_output = tf.matmul(x, w1) + b1
    hidden_output = tf.tanh(z1_output)

with tf.name_scope(name="Output_Layer"):
    w2 = tf.Variable(tf.random_uniform(shape=(500, 10), minval=-1, maxval=1), name="w2")
...
with tf.name_scope(name="Save"):
    ...
    file_writer = tf.summary.FileWriter("Monitoring3")
