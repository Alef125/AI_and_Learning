...
with tf.name_scope(name="Hidden_Layer"):
    ...
    hidden_output = tf.sigmoid(z1_output)

with tf.name_scope(name="Output_Layer"):
    w2 = tf.Variable(tf.random_uniform(shape=(50, 10), minval=-1, maxval=1), name="w2")
    b2 = tf.Variable(tf.zeros(10), name="b2")
    z2_output = tf.matmul(hidden_output, w2) + b2
    output = z2_output
...
with tf.name_scope(name="Save"):
    ...
    file_writer = tf.summary.FileWriter("Monitoring1")
...
with tf.Session() as sess:
    ...
    for i in range(10000):
        ...
        if i % 500 == 0:
            ...
            print ("step %5i accuracy is %.5f and cross_entropy(loss) is %0.5f" % (i, accuracy, current_loss))
