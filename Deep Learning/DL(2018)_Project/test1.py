...
objects = []
with (open("Sample/x_test", "rb")) as openfile:
    while True:
        try:
...
test_images = np.reshape(np.array(objects[0]), newshape=(-1, 32, 32, 1))

'''''
objects = []
with (open("Sample/prediction", "rb")) as openfile:
    while True:
        try:
            objects.append(pickle.load(openfile))
        except EOFError:
...
test_letters = objects[0]
# test_label_one_hot = 1
'''''


def which_letter(code):
...
with tf.Session(config=config) as sess:
    ...
    # test_error = sess.run(accuracy_train, feed_dict={x: test_images, y:te})
    ...
    same_labels = 0
    date_length = len(final_letters)
    # for i in range(date_length):
    #     if final_letters[i] == test_letters[i]:
    #         same_labels = same_labels + 1
    # test_accuracy = float(same_labels) / date_length
    # print 'Accuracy Percent is: ', test_accuracy * 100, '%'

with open('Results/prediction.pkl', 'wb') as f:
    pickle.dump(final_letters, f)
