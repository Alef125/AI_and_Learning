...
# Archive('/content/gdrive/My Drive/train_data.zip').extractall('/content/gdrive/My Drive/Training')
# import pickle

# objects = []
# with (open("/content/gdrive/My Drive/Training/train_data.pickle", "rb")) as openfile:
#    while True:
#        try:
...
# x_data = objects[0]['data']
# y_data = objects[0]['labels']
...
with tf.name_scope('save'):
    ...
    train_accuracy_summary = tf.summary.scalar('training_accuracy', accuracy_train)
    ...
    validation_accuracy_summary = tf.summary.scalar('validation_accuracy', accuracy_train)
    validation_summaries = tf.summary.merge([validation_loss_summary, validation_accuracy_summary])

    # file_writer = tf.summary.FileWriter("Classifier History")
    saver = tf.train.Saver()


def letter_to_one_hot(l):
    one_hot = np.zeros(shape=[26])
    index = ord(l) - ord('A')
    one_hot[index] = 1
    return one_hot
...
with tf.name_scope('prepare_data'):
    x_data, y_data = [1, 1]
    all_images, all_labels = [x_data, y_data]  # Load Data ...
    number_of_data = all_images.shape[0]
    num_validations = 1000
    num_train = number_of_data - num_validations

    # Random validation data:
    v_indeces = random.sample(range(number_of_data), num_validations)

    all_inputs = np.reshape(all_images, newshape=(-1, 32, 32, 1))
    # all_logits = [letter_to_one_hot((x.split('_'))[0]) for x in all_labels]
    all_logits = y_data

    validation_inputs = np.array([all_inputs[v_index][:][:][:] for v_index in v_indeces])
    validation_logits = np.array([all_logits[v_index][:][:][:] for v_index in v_indeces])

    t_indeces = list(set(range(number_of_data)) - set(v_indeces))
    test_inputs = np.array([all_inputs[t_index][:][:][:] for t_index in t_indeces])
    test_logits = np.array([all_logits[t_index][:][:][:] for t_index in t_indeces])


def get_batch(number):
    indeces = random.sample(range(num_train), number)
    x_batch = np.array([test_inputs[index][:][:][:] for index in indeces])
    y_batch = np.array([test_logits[index][:] for index in indeces])
    return x_batch, y_batch


def get_test_batch(number):
    indeces = random.sample(range(num_validations), number)
    x_batch = np.array([validation_inputs[index][:][:][:] for index in indeces])
    y_batch = np.array([validation_logits[index][:] for index in indeces])
    return x_batch, y_batch
...
with tf.Session(config=config) as sess:
    ...
    summary_writer_train = tf.summary.FileWriter('FFF Classifier Train History', sess.graph)
    summary_writer_validation = tf.summary.FileWriter('FFF Classifier Validation History', sess.graph)

    # saver.restore(sess=sess, save_path='./Classifier Model/model.ckpt')

    # Train
    for i in range(itees):
        x_sample, y_sample = get_batch(batch_size)
        ...
        if i % check_iter == 0:
            ...
            summary_writer_train.add_summary(
                (sess.run(train_summaries, feed_dict={x: x_sample, y: y_sample})), i)

            if i % test_iter == 0:
                x_sample, y_sample = get_test_batch(batch_size)
                test_loss, test_accuracy = sess.run((loss, accuracy_train), feed_dict={x: x_sample, y: y_sample})
                print ("Here, On Test Data, accuracy is %.5f and cross_entropy(loss) is %.5f"
                       % (accuracy, current_loss))
                summary_writer_validation.add_summary(
                    sess.run(validation_summaries, feed_dict={x: x_sample, y: y_sample}), i)
                save_path = saver.save(sess, './FFF Classifier Model//model.ckpt')
    ...
    # save_path = saver.save(sess, './Classifier Model//model.ckpt')
